<template>
    <div class="mb-6">
        <VueMultiselect v-model="value" :options="options" placeholder="Selecciona un Evento" label="name" track-by="name"
            @select="obtenerAsistencias">
            <template v-slot:noResult> <span>No se encontraron resultados</span> </template>
        </VueMultiselect>

    </div>
    <input class="mb-5 form-control" type="text" v-model="busqueda" @input="actualizarBusqueda" placeholder="Buscar..." />
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                    <li>
                                        <a class="dropdown-item" data-bs-toggle="tooltip"
                                            ...
                                            title="Ver perfil" :href="route('usuario.perfil', { id: usuario.id })"><i
...
export default {
    ...
    methods: {
        ...
        obtenerAsistencias(evento) {
            axios
                .post(`/api/asistencias/usuario/${this.usuario.id}`, { idEvento: evento.value })
                .then((response) => {
